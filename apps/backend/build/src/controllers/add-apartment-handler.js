...
export async function addApartmentHandler(request, reply) {
    ...
    // insert amenities 
    const amenitiesPromise = Promise.allSettled(apartment.amenities.map(async (amenity) => await AmenitiesRepository.addApartmentAmenity(apartmentId, amenity)));
    // insert pictures
    const picturesPromise = Promise.allSettled(apartment.pictures.map(async (picUrl) => await PicturesRepository.addPicture(apartmentId, picUrl)));
