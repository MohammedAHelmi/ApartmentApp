import { Comparator } from "../types/dal-options.types.js";
 ...
 * @param reply
 */
export async function listApartmentsHandler(request, reply) {
    const options = constructOptions(request.query);
    const matchingApartments = await Promise.allSettled(options.map(async (optionsObj) => await getApartments(optionsObj)));
...
function constructOptions({ term, limit, offset, sortBy, sortOrder }) {
...
function constructSearchFilters(term) {
...
function constructSort(sortBy, sortOrder = "asc") {
...
async function getApartments(options) {
    const apartments = await ApartmentsRepository.getApartments(options);
    await attachPictures(apartments);
    return apartments;
}
async function attachPictures(apartments) {
    const results = await Promise.allSettled(apartments.map((apt) => PicturesRepository.getPictures(apt.id)));
